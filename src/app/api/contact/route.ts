...
export async function POST(request: Request) {
  try {
    ...
    // Support both old format (expertId) and new format (to, type)
    const { 
      expertId, 
      senderName, 
      senderEmail, 
      message,
      // New format for companies
      to,
      fromName,
      fromEmail,
      companyName,
    ...
    if (type === "company" && to && fromName && fromEmail && message) {
      const { error } = await resend.emails.send({
        from: "Greybird <hello@greybird.pro>",
        to: to,
        replyTo: fromEmail,
        subject: `New message from ${fromName} via Greybird`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #334155; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>You've received a new message through your Greybird company profile:</p>
            
            <div style="background: #f1f5f9; border-radius: 8px; padding: 20px; margin: 25px 0;">
              <p style="margin: 0 0 10px;"><strong>From:</strong> ${fromName}</p>
              <p style="margin: 0 0 10px;"><strong>Email:</strong> <a href="mailto:${fromEmail}" style="color: #1e293b;">${fromEmail}</a></p>
            ...
            <p style="color: #94a3b8; font-size: 12px; text-align: center;">
              This message was sent via your Greybird company profile<br>
